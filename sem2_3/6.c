...
int captainMode (int sem_id, int countRides, int countSeats)
{
    ...
    if (captPid == 0) {  // child
    printf ("Captain created. Have a good time! [%d]\n", getpid ());

    for (int i = 0; i < countRides; ++i) {
        printf ("Captain's ride No.%d begins.\n", i + 1);
        if (semop (sem_id, &landing[0], 1) == -1) {
            perror ("Captain ride begin error");
            return -1;
        }

        sleep (1);

        printf ("Captain's ride No.%d ends.\n", i + 1);
        if (semop (sem_id, &debarkation[1], 1) == -1) {
            perror ("Captain ride end error");
            return -1;
        }
    }

    printf ("Captain's work finished.\n");
    ...
    for (int i = 0; i < countRides; ++i) {
        printf ("\t\t\t\t\t Yacht loading No.%d\n", i + 1);
        if (semop (sem_id, &landing[1], 1) == -1) {
            perror ("Loading error");
            return -1;
        }

        printf ("\t\t\t\t\t Yacht loaded and goes\n");
        sleep (2);

        printf ("\t\t\t\t\t Yacht unloaded\n");
        if (semop (sem_id, &debarkation[0], 1) == -1) {
            perror ("Unloading error");
            return -1;
        }
    }
}
